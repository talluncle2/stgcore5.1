...
export function AuthCallback() {
  ...
  useEffect(() => {
    ...
    async function handleCallback() {
      ...
      const isModerator = params.get("is_moderator");
      const role = params.get("role");

      if (import.meta.env.DEV) {
        console.log("Auth callback recebeu token:", token ? "sim" : "nao");
        console.log("Cargo detectado", { is_admin: isAdmin, is_moderator: isModerator, role });
      }
      ...
      <div className="stg-auth-panel tactical-panel tactical-edge relative max-w-md p-8 text-center">
        ...
        {!error && (
          <>
            ...
            <p className="mt-4 text-sm text-[#9ca58d]">Verificando sessão com Discord...</p>
          </>
        )}
        {error && (
